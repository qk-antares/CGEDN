...
class MinCostHeap:
    ...
    def push(self, path, cost):
        if cost is not None:
            heapq.heappush(self.heap, (cost, next(self.counter), path))
        else:
            raise ValueError("Cost cannot be None")
    
    def pop(self):
        if self.heap:
            # pop (cost, counter, path)
...
def beam_search(graph1, graph2, beamsize):
    ...
    # create a min cost heap to store search paths and total costs (total_cost = p_cost + h_cost)
    OPEN = MinCostHeap()
    ...
    for v in pending_v:
        ...
        OPEN.push(edit_path, total_cost)
    ...
    while OPEN.size():
        if OPEN.size() > beamsize * 1.2:
            OPEN.resize(beamsize)

        # get minimum-cost partial edit path min_path and its cost from OPEN
        cost, _, min_path  = OPEN.pop()
