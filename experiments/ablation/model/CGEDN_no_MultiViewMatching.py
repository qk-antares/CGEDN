...
from experiments.ablation.model.common.MatchingModule import MatchingModule
from experiments.ablation.model.common.TensorNetworkModule import TensorNetworkModule


class CGEDN_no_MultiViewMatching(nn.Module):
    def __init__(self, args, node_dim, edge_dim):
        super(CGEDN_no_MultiViewMatching, self).__init__()
        self.args = args
    ...
    def setup_layers(self):
        ...
        self.mapping_matrix = MatchingModule(d=gnn_filters[-1])
        self.cost_matrix = MatchingModule(d=gnn_filters[-1])
    ...
    def forward(self, data):
        ...
        emb1, emb2 = self.forward_inter_gconv_layers(
            emb1, edge_index1, edge_attr1, emb2, edge_index2, edge_attr2
        )

        mapping_matrix = self.mapping_matrix(emb1, emb2)
        mapping_matrix = F.softmax(mapping_matrix, dim=1)
        cost_matrix = self.cost_matrix(emb1, emb2)

        soft_matrix = mapping_matrix * cost_matrix
        bias_value = self.get_bias_value(emb1, emb2)
