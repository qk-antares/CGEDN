...
from experiments.ablation.model.common.IntraGConv import IntraGConv
...
class CGEDN_no_InterGConv(nn.Module):
    def __init__(self, args, node_dim, edge_dim):
        super(CGEDN_no_InterGConv, self).__init__()
        self.args = args
    ...
    def setup_layers(self):
        ...
        for i in range(0, self.gnn_layers):
            setattr(self, "gnn{}".format(i + 1), IntraGConv(**gnn_settings[i]))
    ...
    def forward_intra_gconv_layers(
        self, emb1, edge_index1, edge_attr1, emb2, edge_index2, edge_attr2
    ):
        embs1 = []
        embs2 = []
        for i in range(1, self.gnn_layers + 1):
            intra_gconv = getattr(self, "gnn{}".format(i))
            emb1, emb2 = intra_gconv(emb1, edge_index1, edge_attr1), intra_gconv(emb2, edge_index2, edge_attr2)
            embs1.append(emb1)
            embs2.append(emb2)
        return embs1, embs2
    ...
    def forward(self, data):
        ...
        embs1, embs2 = self.forward_intra_gconv_layers(
            emb1, edge_index1, edge_attr1, emb2, edge_index2, edge_attr2
        )
